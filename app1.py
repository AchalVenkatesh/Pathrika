...
def login():
    ...
    if role == "admin":
        if is_Admin_valid(connection, username, password):
    ...
    # Check for other roles (teacher and superintendent)
    ...
    # Fallback in case no conditions were met
    flash('Role not recognized or no valid credentials provided.')
...
def admin_dashboard():
    ...
    private_key = isPrivateKey(connection,username)
    if not private_key:
...
def key_gen():
    key_phrase = request.json['keyPhrase']
    swarm_key = request.json['encrypted_swarmKey']
    username = session.get('username')
    # phrase_bytes = key_phrase.encode('utf-8')
# Generate a salt
    save_keys(connection,key_phrase, swarm_key, username)
    # save_swarm_key(connection, swarm_key)
...
def teacher_dashboard():
    ...
    join_network = joined_network(connection, username)
    return render_template('teacher_dashboard.html', requests=teacher_requests, username=username, join_network=join_network)
...
def fetch_pdf(unique_code):
    try:
        ...
        result, isReleased = retrieve_hash(unique_code, teacher_id)
        print("hash", result)
        print(isReleased)
